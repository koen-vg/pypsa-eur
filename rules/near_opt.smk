...
near_opt_mem = config["solving"]["mem_mb"] * config["near_opt"].get("num_parallel", 1)
...
rule near_opt_perfect:
    ...
    benchmark:
        (
            RESULTS
            + "benchmarks/solve_sector_network/elec_s{simpl}_{clusters}_l{ll}_{opts}_{sector_opts}_near_opt"
        )
    conda:
