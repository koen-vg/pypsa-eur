...
vars_to_store = ["lifetime", "capital_cost"]  # In addition to 'attr' and 'attr_max'
...
def aggregate_build_years(n):
    ...
    for c in n.iterate_components():
        ...
        if ("build_year" in c.df.columns) and (c.df.build_year > 0).any():
            ...
            tol = 0
            c.df.loc[non_extendable, f"{attr}_min"] = (1 - tol) * c.df.loc[
                non_extendable, attr
            ]
            c.df.loc[non_extendable, f"{attr}_max"] = (1 + tol) * c.df.loc[
                non_extendable, attr
            ]
...
if __name__ == "__main__":
    ...
    with memory_logger(
        filename=getattr(snakemake.log, "memory", None), interval=0.01
    ) as mem:
